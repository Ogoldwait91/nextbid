...
class PairingsOverview extends StatelessWidget {
  ...
  const PairingsOverview({super.key, required this.pairings, required this.avgCredit});
  ...
  Widget build(BuildContext context) {
    ...
    final topRegions = regions.entries.toList()
    ...
    return Card(
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text("Pairings overview", style: TextStyle(fontWeight: FontWeight.w600)),
          ...
          Wrap(
            spacing: 8,
            runSpacing: -6,
            children: topRegions.take(3).map((e) => Chip(
              label: Text("${e.key} • ${e.value}"),
              visualDensity: VisualDensity.compact,
            )).toList(),
          ),
          const SizedBox(height: 8),
          if (show.isEmpty) const Text("No pairings available")
          else ...show.map((p) {
            final id = p["id"] ?? "PAIR";
            final cr = p["credit"] ?? "?";
            final ngt = p["nights"] ?? "?";
